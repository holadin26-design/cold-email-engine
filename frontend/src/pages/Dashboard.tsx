...
export default function Dashboard() {
    ...
    const loadData = async () => {
        try {
            // Fetch stats and recent followups from the local backend
    ...
    const processQueue = async () => {
        setProcessing(true);
        try {
            const { data } = await api.post(`/queue/process`);
            toast.success(`Processed: ${data?.processed || 0} sent, ${data?.failed || 0} failed`);
            loadData();
        } catch (err: any) {
            toast.error(err.response?.data?.error || "Failed to process queue");
        } finally {
            setProcessing(false);
        }
    };
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader className="flex-row items-center justify-between py-4">
                    ...
                    <Button variant="ghost" size="sm" className="text-xs text-primary h-8" onClick={() => navigate("/campaigns")}>
                        View Campaigns <ArrowUpRight className="ml-1 h-3 w-3" />
                    </Button>
                </CardHeader>
